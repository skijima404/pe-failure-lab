import type { ModelAdapterRequest, ModelAdapterResponse } from "../execution/runtime-responder.ts";
...
function renderSidecarCandidateResponse(sessionId: string, speakerId: string, language: string, candidate: SidecarReactionCandidateLike): string | null {
...
function renderGenericResponse(sessionId: string, speakerId: string, language: string, activeTopic: string, coreConcern: string): string {
  ...
  return `Verification actor response from ${speakerId} on ${activeTopic}. Concern: ${coreConcern}.`;
}

export function renderLocalActorText(request: ModelAdapterRequest): string {
  ...
  const runtimePersona =
    prompt && typeof prompt === "object" && "runtime_persona" in prompt
      ...
      : null;
  const coreConcern = typeof runtimePersona?.core_concern === "string" ? runtimePersona.core_concern : "the current topic";
  const activeTopic = prompt && typeof prompt === "object" && "active_topic" in prompt ? extractTopicLabel(prompt.active_topic) : "the active topic";
  const activeWhisper = prompt && typeof prompt === "object" && "active_whisper" in prompt ? (prompt.active_whisper as SidecarReactionCandidateLike | null) : null;
  ...
  return renderGenericResponse(request.session_id, request.speaker_id, language, activeTopic, coreConcern);
}

export function buildLocalActorResponseMetadata(
  request: ModelAdapterRequest,
  transport: string,
  renderingMode: string,
): ModelAdapterResponse["metadata"] {
  return {
    mock: transport === "mock-model-adapter",
