...
import { loadRepoEnv, parseReasoningEffort, requiredEnv } from "../shared/_env.mjs";

function parseArgs(argv) {
  let adapterName = "live";
  let language = "en";
  const remaining = [];
...
function createResponder(adapterName) {
  ...
  return new LocalLiveActorResponder();
}
...
function printHeader(adapterName, language) {
  if (language.startsWith("ja")) {
    ...
    console.log("==================");
    console.log("既定モード: ローカルファースト runtime の live actor ウォークスルー。");
    console.log(
      `アクター生成モード: ${adapterName === "openai" ? "OpenAI リモートアダプタ（任意）" : "ローカル live actor generation（既定）"}`,
    );
    console.log("");
    return;
  }
  ...
  console.log("==============");
  console.log("Default mode: local-first runtime live actor walkthrough.");
  console.log(
    `Actor generation mode: ${adapterName === "openai" ? "remote-backed OpenAI adapter (optional)" : "local live actor generation (default)"}`,
  );
  console.log("");
}
